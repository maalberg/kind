...
def advantage(model, policy, reward_fn, value_fn, normalizer, replay, replay_data, horizon, gamma, epoch=0):
...
class iteration_improve(iteration_state):
    ...
    def iterate(self, replay_nom, replay_exc, dataset):
        loss = self._improve_policy2(dataset, replay_nom + replay_exc, self.iter.reward_fn_nom, self.iter.reward_fn_exc)
        self.iter._set_state(self.iter._get_state_evaluate())

        return loss

    def _improve_policy2(self, dataset, replay, reward_fn_nom, reward_fn_exc):
    ...
    def _improve_policy(self, dataset, replay, reward_fn_nom, reward_fn_exc):
        ...
        for epoch in range(nepoch):
            ...
            for k in range(horizon):
                ...
                # --! compute action
                delta_u = res_policy.forward(state, zeta=zeta, epoch=epoch)
